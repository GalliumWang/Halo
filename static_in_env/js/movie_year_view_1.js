function show() {
    ...
    function update(year) {
        ...
        //console.log(adjustedData[yearIndex]);

        var adjustedIndexedData = adjustedData.map(function (d) { return mapToindexed(d, adjustedData[104]) });
        var unadjustedCleaned = unadjustedData.map(mapToIncome);

        //console.log(adjustedIndexedData);
        ...
        var yIncomeScale = d3.scaleLinear()
            .range([height, 0])
            .domain([
                // Math.floor(incomeMin),
                // Math.ceil(incomeMax)
                Math.floor(incomeMin / 100) * 100,
                Math.ceil(incomeMax / 100) * 100
            ]);
    ...
    function addAxis(yIncomeScale, yIndexedScale, xScale, xRangeAdjusted) {
        ...
        var steps = d3.range(100 - xRangeAdjusted, 100 + xRangeAdjusted + 1, 10);
        ...
        var bottomAxis = d3.axisBottom().scale(xScale).ticks(20, "r");
    ...
    function addMouseTracker(xScale, yIndexedScale, yIncomeScale, adjustedIndexedData, unadjustedCleaned) {
        ...
        function mousemove() {
            ...
            var d = adjustedIndexedData[xToShow - 1914];
            var dIncome = unadjustedCleaned[xToShow - 1914];
