...
class DictClassifier:
    def __init__(self):
        ...
        self.__root_filepath = "./add_on/SentimentPolarityAnalysis/spa/f_dict/"
        jieba.load_userdict(
            "./add_on/SentimentPolarityAnalysis/spa/f_dict/user.dict")  # 准备分词词典
        # TODO self define path

        # 准备情感词典词典
        self.__phrase_dict = self.__get_phrase_dict()
        self.__positive_dict = self.__get_dict(
            self.__root_filepath + "positive_dict.txt")
        self.__negative_dict = self.__get_dict(
            self.__root_filepath + "negative_dict.txt")
        self.__conjunction_dict = self.__get_dict(
            self.__root_filepath + "conjunction_dict.txt")
        self.__punctuation_dict = self.__get_dict(
            self.__root_filepath + "punctuation_dict.txt")
        self.__adverb_dict = self.__get_dict(
            self.__root_filepath + "adverb_dict.txt")
        self.__denial_dict = self.__get_dict(
            self.__root_filepath + "denial_dict.txt")
    ...
    def analysis_file(self, filepath_in, filepath_out, encoding="utf-8", print_show=False, start=0, end=-1):
        ...
        with open(filepath_in, "r", encoding=encoding) as f:
            ...
            for line in f:
                ...
                results.append(self.analyse_sentence(
                    line.strip(), filepath_out, print_show))
    ...
    def analyse_sentence(self, sentence, runout_filepath=None, print_show=False):
        ...
        for i in range(len(the_clauses)):
            # 情感分析子句的数据结构
            sub_clause = self.__analyse_clause(
                the_clauses[i].replace("。", "."), runout_filepath, print_show)
        ...
        if runout_filepath is not None:
            ...
            self.__write_runout_file(
                runout_filepath, str(comment_analysis) + "\n\n\n\n")
        if print_show:
            print("\n" + sentence)
    ...
    def __analyse_clause(self, the_clause, runout_filepath, print_show):
        sub_clause = {"score": 0, "positive": [], "negative": [],
                      "conjunction": [], "punctuation": [], "pattern": []}
        ...
        for i in range(len(seg_result)):
            mark, result = self.__analyse_word(
                seg_result[i].word, seg_result, i)
            if mark == 0:
                continue
            elif mark == 1:
    ...
    def __is_word_conjunction(self, the_word):
        if the_word in self.__conjunction_dict:
            conjunction = {"key": the_word,
                           "value": self.__conjunction_dict[the_word]}
            return conjunction
        return ""

    def __is_word_punctuation(self, the_word):
        if the_word in self.__punctuation_dict:
            punctuation = {"key": the_word,
    ...
    def __emotional_word_analysis(self, core_word, value, segments, index):
        # 在情感词典内，则构建一个以情感词为中心的字典数据结构
        orientation = {"key": core_word, "adverb": [],
                       "denial": [], "value": value}
...
class KNNClassifier:
    ...
    def __get_sorted_distances(self, input_data):
        ...
        diff_mat = np.tile(input_data_vector,
                           (size[0], 1)) - self.__train_data_vectors
...
class BayesClassifier:
    ...
    def _train(self, train_data, train_data_labels, best_words=None):
        ...
        for word in total_word:
            self._pos_word_p[word] = np.log(
                total_pos_data.get(word, 1e-100) / total_pos_length)
            self._neg_word_p[word] = np.log(
                total_neg_data.get(word, 1e-100) / total_neg_length)
...
class MaxEntClassifier:
    ...
    def calculate_probability(self, features):
        weights = [(self.prob_weight(features, label), label)
